...
COM_QUANTIFIERS = "(处|台|架|枚|趟|幅|平|方|堵|间|床|株|批|项|例|列|篇|栋|注|亩|封|艘|把|目|套|段|人|所|朵|匹|张|座|回|场|尾|条|个|首|阙|阵|网|炮|顶|丘|棵|只|支|袭|辆|挑|担|颗|壳|窠|曲|墙|群|腔|砣|座|客|贯|扎|捆|刀|令|打|手|罗|坡|山|岭|江|溪|钟|队|单|双|对|出|口|头|脚|板|跳|枝|件|贴|针|线|管|名|位|身|堂|课|本|页|家|户|层|丝|毫|厘|分|钱|两|斤|担|铢|石|钧|锱|忽|(千|毫|微)克|毫|厘|(公)分|分|寸|尺|丈|里|寻|常|铺|程|(千|分|厘|毫|微)米|米|撮|勺|合|升|斗|石|盘|碗|碟|叠|桶|笼|盆|盒|杯|钟|斛|锅|簋|篮|盘|桶|罐|瓶|壶|卮|盏|箩|箱|煲|啖|袋|钵|年|月|日|季|刻|时|周|天|秒|分|小时|旬|纪|岁|世|更|夜|春|夏|秋|冬|代|伏|辈|丸|泡|粒|颗|幢|堆|条|根|支|道|面|片|张|颗|块|元|(亿|千万|百万|万|千|百)|(亿|千万|百万|万|千|百|美|欧|)元|(亿|千万|百万|万|千|百|十|)吨|(亿|千万|百万|万|千|百|)块|角|毛|分|英镑|人民币|(毫)升)"
COM_QUANTIFIERS = "(處|台|架|枚|趟|幅|平|方|堵|間|床|株|批|項|例|列|篇|棟|注|畝|封|艘|把|目|套|段|人|所|朵|匹|張|座|回|場|尾|條|個|首|闕|陣|網|炮|頂|丘|棵|只|支|襲|輛|挑|擔|顆|殼|窠|曲|牆|群|腔|砣|座|客|貫|扎|捆|刀|令|打|手|羅|坡|山|嶺|江|溪|鍾|隊|單|雙|對|出|口|頭|腳|板|跳|枝|件|貼|針|線|管|名|位|身|堂|課|本|頁|家|戶|層|絲|毫|厘|分|錢|兩|斤|擔|銖|石|鈞|錙|忽|(千|毫|微)克|毫|厘|(公)分|分|寸|尺|丈|里|尋|常|鋪|程|(千|分|厘|毫|微)米|米|撮|勺|合|升|斗|石|盤|碗|碟|疊|桶|籠|盆|盒|杯|鍾|斛|鍋|簋|籃|盤|桶|罐|樽|壺|卮|盞|籮|箱|煲|啖|袋|缽|年|月|日|季|刻|時|周|天|秒|分|小時|旬|紀|歲|世|更|夜|春|夏|秋|冬|代|伏|輩|丸|泡|粒|顆|幢|堆|條|根|支|道|面|片|張|顆|塊|元|(億|千祈|百萬|萬|千|百)|(億|千祈|百萬|萬|千|百|美|歐|)元|(億|千祈|百萬|萬|千|百|十|)噸|(億|千祈|百萬|萬|千|百|)塊|角|毛|分|英鎊|人民幣|(毫)升)"
...
def replace_percentage(match) -> str:
    ...
    if symbol == "%":
        ...
        unit_text = ""
    result = f"{sign}{unit_text}{percent}"
    return result
...
#RE_ASMD = re.compile(
    #r"(?:[^\s\d]*-?(?:[A-Za-z]*\d+(?:\.\d+)?|\.\d+)[⁰¹²³⁴⁵⁶⁷⁸⁹ˣʸⁿ]*[^\s\d]*\s*[\+\-\×÷=]\s*)+"
    #r"[^\s\d]*-?(?:[A-Za-z]*\d+(?:\.\d+)?|\.\d+)[⁰¹²³⁴⁵⁶⁷⁸⁹ˣʸⁿ]*[^\s\d]*"
#)

asmd_map = {"+": "加", "-": "減", "×": "乘", "÷": "除以", "=": "等於", ":": "比"}
...
RE_LEFT_QUANTIFIERS = re.compile(r"(花費|消費|價格)(\d+)")
...
def replace_positive_quantifier(match) -> str:
    ...
    #number: str = num2str(number)
    print(number)
    number_cn = an2cn(number, "low")
    #number_cn = "两" if number_cn == "二" else number_cn
    number_cn = number_cn.replace("万","萬").replace("亿","億")
    result = f"{number_cn}{match_2}{quantifiers}"
    return result

def replace_number_with_chinese(match) -> str:
    """
    将匹配到的阿拉伯数字转换为中文大写表达
    例："花费35000" → "花费三万五千"
    """
    prefix = match.group(1) or ""   # 比如 "花费"
    number = match.group(2)         # 比如 "35000"

    # 使用 cn2an 库进行数字转中文
    number_cn = an2cn(number, "low")  # 输出如 "三万五千"
    number_cn = number_cn.replace("万","萬").replace("亿","億")

    return f"{prefix}{number_cn}"
...
#RE_TO_RANGE = re.compile(
    #r"((-?)((\d+)(\.\d+)?)|(\.(\d+)))"
    #r"(%|°C|℃|度|摄氏度|cm2|cm²|cm3|cm³|cm|db|ds|kg|km|m2|m²|m³|m3|ml|m|mm|s|h|年|km/h)"
    #r"(~|-)"   # allow ~ or - but only if no "=" appears later
    #r"((-?)((\d+)(\.\d+)?)|(\.(\d+)))"
    #r"(%|°C|℃|度|摄氏度|cm2|cm²|cm3|cm³|cm|db|ds|kg|km|m2|m²|m³|m3|ml|m|mm|s|h|年|km/h)"
    #r"(?![^=]*=)"
#)
...
UNIT = r'(?i:%|°C|℃|度|攝氏度|cm2|cm²|cm3|cm³|cm|db|ds|kg|km|m2|m²|m³|m3|ml|m|mm|s|h|年|km/h|l|T|J|W|kg/L|m/s)'

RE_TO_RANGE = re.compile(
    rf'({NUM}){UNIT}\s*(~|-)\s*({NUM}){UNIT}(?!=)'
)
...
def replace_to_range(match) -> str:
    ...
    Returns:
        str
    """
    result = match.group(0).replace("~", "至").replace("-", "至")
...
def num2str(value_string: str) -> str:
    ...
    if decimal:
        ...
        result += "點" + verbalize_digit(decimal)
    return result
