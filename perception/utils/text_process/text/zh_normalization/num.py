...
RE_LEFT_QUANTIFIERS = re.compile(r"(花费|消费|价格)(\d+)")
...
def replace_positive_quantifier(match) -> str:
    ...
    number_cn = number_cn.replace("二万","两万").replace("二千","两千").replace("二百","两百").replace("二亿","两亿").replace("二兆","两兆")
...
UNIT = r'(?i:%|°C|℃|度|摄氏度|cm2|cm²|cm3|cm³|cm|db|ds|kg|km|m2|m²|m³|m3|ml|m|mm|s|h|年|km/h|l|T|J|W|kg/L|m/s)'
...
RE_SIMPLE_RATIO = re.compile(
    r"(配比|比例|比分|汇率|比值)[^:，。、“”‘’【】（）()、,.;!?！？]*?(?<=\d):(?=\d)"
)
