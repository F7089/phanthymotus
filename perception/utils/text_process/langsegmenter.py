...
class LangSegmenter():
    ...
    def getTexts(text,default_lang = ""):
        splitter_cls = _get_lang_splitter_cls()
        if splitter_cls is None:
            # Offline / missing deps: treat whole string as default language.
            lang = default_lang or "zh"
            if full_en(text):
                lang = "en"
            return [{"lang": lang, "text": text}]

        lang_splitter = splitter_cls(lang_map=LangSegmenter.DEFAULT_LANG_MAP)
